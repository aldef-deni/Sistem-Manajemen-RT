...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">Edit data KK — {{ $kartu_keluarga->no_kk }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('kartu-keluarga.update', $kartu_keluarga) }}" id="kkForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-6">
                        ...
                        <input type="text" name="no_kk" value="{{ old('no_kk', $kartu_keluarga->no_kk) }}" inputmode="numeric" pattern="[0-9]{16}" maxlength="16" required autocomplete="off"
                    ...
                    <div class="md:col-span-3">
                        ...
                        <input type="text" name="rt" value="{{ old('rt', $kartu_keluarga->rt) }}" maxlength="5"
                               placeholder="001"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                    <div class="md:col-span-3">
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">RW</label>
                        <input type="text" name="rw" value="{{ old('rw', $kartu_keluarga->rw) }}" maxlength="5"
                               placeholder="001"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="alamat" rows="2" required
                              ...
                              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all resize-none">{{ old('alamat', $kartu_keluarga->alamat) }}</textarea>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Desa / Kelurahan</label>
                        <input type="text" name="desa" value="{{ old('desa', $kartu_keluarga->desa) }}"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Kecamatan</label>
                        <input type="text" name="kecamatan" value="{{ old('kecamatan', $kartu_keluarga->kecamatan) }}"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Kabupaten / Kota</label>
                        <input type="text" name="kabupaten" value="{{ old('kabupaten', $kartu_keluarga->kabupaten) }}"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Kode Pos</label>
                        <input type="text" name="kode_pos" value="{{ old('kode_pos', $kartu_keluarga->kode_pos) }}" maxlength="10"
                               class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-start gap-4">
                        <div id="kkPreview" class="w-28 h-20 shrink-0 rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 flex items-center justify-center overflow-hidden">
                            @if($kartu_keluarga->file_kk_url)
                                @if($kartu_keluarga->file_kk_is_image)
                                    <img src="{{ $kartu_keluarga->file_kk_url }}" class="w-full h-full object-cover" alt="KK">
                                @else
                                    <span class="text-[11px] text-amber-600 font-semibold px-2 text-center">📄 PDF</span>
                                @endif
                            @else
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-[11px] text-slate-400 mt-1.5">Format: JPG, JPEG, PDF. Maksimal 5 MB. Pilih file baru untuk mengganti.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
            ...
            <div class="p-6">
                        ...
                        <tbody id="anggotaBody">
                            @foreach ($kartu_keluarga->anggota as $i => $a)
                                <tr class="anggota-row border-b border-slate-100">
                                    ...
                                    <td class="px-3 py-2">
                                        <input type="text" name="anggota[{{ $i }}][nik]" value="{{ $a->nik }}" inputmode="numeric" pattern="[0-9]{16}" maxlength="16" required autocomplete="off" placeholder="16 digit"
                                    ...
                                    <td class="px-3 py-2">
                                        <input type="text" name="anggota[{{ $i }}][nama_lengkap]" value="{{ $a->nama_lengkap }}" required placeholder="Nama lengkap"
                                               class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500">
                                    </td>
                                    <td class="px-3 py-2">
                                        <input type="text" name="anggota[{{ $i }}][no_hp]" value="{{ $a->no_hp }}" placeholder="08xx..."
                                               class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500">
                                    </td>
                                    <td class="px-3 py-2">
                                    ...
                                    <td class="px-3 py-2">
                                        <input type="date" name="anggota[{{ $i }}][tanggal_lahir]" value="{{ $a->tanggal_lahir?->format('Y-m-d') }}"
                                               class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500">
                                    </td>
                                    <td class="px-3 py-2">
                                        @if ($i === 0)
                                            <input type="text" name="anggota[{{ $i }}][status_hubungan]" value="Kepala Keluarga" readonly
                                        ...
                                        @else
                                            <select name="anggota[{{ $i }}][status_hubungan]"
                                                    class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500">
                                                @foreach(['Istri','Anak','Orang Tua','Mertua','Adik','Kakak','Lainnya'] as $opt)
                                                    <option value="{{ $opt }}" {{ $a->status_hubungan === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                                @endforeach
                                            </select>
                                        @endif
                                    </td>
                                    <td class="px-3 py-2">
                                        <select name="anggota[{{ $i }}][domisili]"
                                                class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500">
                                            @foreach(['Tetap','Kontrakan','Kos'] as $opt)
                                                <option value="{{ $opt }}" {{ $a->domisili === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="px-3 py-2">
                                        <button type="button" onclick="removeAnggota(this)" class="w-7 h-7 rounded-lg bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-100 transition-colors" title="Hapus" {{ $i === 0 ? 'disabled' : '' }}>
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-4 flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                    ...
                    <p class="text-xs text-blue-600">Hanya <strong>Kepala Keluarga</strong> yang dapat membuat akun Warga melalui <strong>{{ url('/daftar-warga') }}</strong>. Perubahan NIK atau Nomor KK akan menjadi identitas verifikasi berikutnya.</p>
                </div>
            </div>
        </div>
...
@push('scripts')
<script>
let anggotaIndex = {{ $kartu_keluarga->anggota->count() }};
