...
<div class="space-y-6">
    ...
    <form method="GET" action="{{ route('akun.index') }}">
        <div class="relative max-w-md">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari akun atau data warga..."
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50/80">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Data Warga</th>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                @if ($user->anggotaKeluarga)
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">
                                        ...
                                        {{ $user->anggotaKeluarga->nama_lengkap }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <a href="{{ route('akun.edit', $user) }}" class="w-7 h-7 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center hover:bg-amber-100 transition-colors" title="Edit akun & password">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    @else
                                    <span class="w-7 h-7 rounded-md bg-slate-50 text-slate-300 flex items-center justify-center cursor-not-allowed" title="Akun Administrator dilindungi">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
